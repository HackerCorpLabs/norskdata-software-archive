...
function entryToYamlDoc(entry: CatalogEntry, rootDir: string): Record<string, unknown> {
  ...
  // Photos - only DISK photos live in the floppy YAML. Set photos and the
  // set-level label transcription belong to the product+version GROUP folder
  // (collections/{slug}/) and are attached at load time, never written here.
  const photos: Record<string, unknown> = { disk: [], set: [] };
  if (entry.storage?.git) {
    const git = entry.storage.git;
    if (yamlDir && git.diskPhotos?.length) {
      photos.disk = git.diskPhotos.map(p => {
        const absP = join(rootDir, p);
        return relative(yamlDir, absP);
      });
    }
  }
  doc.photos = photos;
...
/**
 * Save the catalog to disk (legacy JSON format).

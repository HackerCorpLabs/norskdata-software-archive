...
import { loadCatalog, saveCatalog, loadProducts, saveProducts, saveProductYaml, saveFloppyYaml, generateCatalogJson, findByVolumeName, findByMd5 } from './api/catalog.js';
...
import { generateLegacyFloppiesJson } from './api/compat-export.js';
...
app.post('/api/match/confirm', async (req, res) => {
  try {
    ...
    await saveCatalog(ROOT_DIR, cat);
    catalog = cat; // refresh cache
...
app.post('/api/products/rename-id', async (req, res) => {
  try {
    ...
    // 4. Persist: floppies.json from memory, then regenerate both JSON files from YAML (source of truth)
    await saveCatalog(ROOT_DIR, cat);
    await generateCatalogJson(ROOT_DIR);
    catalog = cat;
    productsCache = null;
...
app.post('/api/git/push', async (_req, res) => {
  try {
    const output = execSync('git push origin main 2>&1', { cwd: ROOT_DIR }).toString();
...
// ============================================================
// Compat export endpoint
// ============================================================

app.get('/api/export/floppies.json', async (_req, res) => {
  try {
    const cat = await getCatalog();
    await generateLegacyFloppiesJson(ROOT_DIR, cat);
    const data = await readFile(join(ROOT_DIR, 'floppies.json'), 'utf-8');

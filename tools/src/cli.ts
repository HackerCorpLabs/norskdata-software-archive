...
import { loadCatalog, saveCatalog, findByMd5, findByVolumeName, generateCatalogJson, loadProducts, saveProductYaml, consolidateGroupPhotos } from './api/catalog.js';
...
// --- export-compat ---
program
  .command('export-compat')
  .description('Generate legacy floppies.json in repo root for ndfloppy app compatibility')
  .action(async () => {
    const rootDir = getRepoRoot();
    try {
      const catalog = await loadCatalog(rootDir);
      await generateLegacyFloppiesJson(rootDir, catalog);
    } catch (err) {
